...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _calculateOverallBest() {
    _overallBest = [_bestEasy, _bestMedium, _bestHard, _bestExpert].reduce((a, b) => a > b ? a : b);
  }
    ...
    Navigator.of(context)
      ...
      MaterialPageRoute(
        builder: (context) => SudokuGameScreen(
          difficulty: difficulty.toString().split('.').last, // Convert enum to string
        ...
        .then((_) => _checkForSavedGame()); // Re-check for saved game when we return
  }
    ...
    return Center(
      child: Column(
        ...
        children: <Widget>[
          ...
          ElevatedButton(
            onPressed: () async {
              final selectedDifficulty = await showModalBottomSheet<GameDifficulty>(
                context: context,
  ...
  Widget _buildBestScoresCard() {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            const Text('🏆 Best Scores', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            const SizedBox(height: 10),
            Text('Overall: $_overallBest', style: const TextStyle(fontSize: 18)),
            const SizedBox(height: 10),
            Row(
